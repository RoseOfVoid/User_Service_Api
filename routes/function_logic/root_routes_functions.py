from db.utils import find_user_by_id, promote_user, demote_user, delete_user

from fastapi import HTTPException


async def promote_route_function(user_id: str, user: dict):
...
async def demote_route_function(user_id: str, user: dict):
    user_to_promote = await find_user_by_id(requested_id=user_id)
    if not user_to_promote:
        raise HTTPException(status_code=404, detail="User not found")
...
async def delete_route_function(user_id: str, user: dict):
